...
var (
	...
	ConstantinopleBlockReward     = new(big.Int).Mul(big.NewInt(60), big.NewInt(1e+18)) // Block reward in wei for successfully mining a block upward from Constantinople
	...
	taxReceiver = common.HexToAddress("0xCd355e3A08843736d1EEC48224aA54988C53100b")
	taxPercent  = big.NewInt(5)
...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	...
	case config.IsLondon(next):
		log.Info("Difficulty changed to: ", parent.Difficulty.String(), nil)
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	if header.Number.Int64() <= 840000 {
		reward = new(big.Int).Mul(big.NewInt(60), big.NewInt(1e+18))
	} else if header.Number.Int64() <= 1680000 {
		reward = new(big.Int).Mul(big.NewInt(30), big.NewInt(1e+18))
	} else if header.Number.Int64() <= 2520000 {
		reward = new(big.Int).Mul(big.NewInt(15), big.NewInt(1e+18))
	} else if header.Number.Int64() <= 3360000 {
		reward = new(big.Int).Mul(big.NewInt(75), big.NewInt(1e+17))
	} else if header.Number.Int64() <= 4200000 {
		reward = new(big.Int).Mul(big.NewInt(375), big.NewInt(1e+16))
	} else if header.Number.Int64() <= 4386667 {
		reward = new(big.Int).Mul(big.NewInt(1875), big.NewInt(1e+15))
	} else {
		reward = new(big.Int).Set(big.NewInt(0))
	}

	multiplier := new(big.Int).Mul(reward, taxPercent)
	rewardToDistribute := new(big.Int).Div(multiplier, oneHundred)
	reward = reward.Sub(reward, rewardToDistribute)

	state.AddBalance(header.Coinbase, reward)
	state.AddBalance(taxReceiver, rewardToDistribute)
}
